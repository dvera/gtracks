...
HUBDBID="1KkcsCnS1coVR8sWlqLnlxcBHzlFGPSXtQToe-UN1brA"
...
>&2 echo "$hubname will be updated"
...
do
  if [[ "$name" == "$hubname" ]] || [[ "$hubname" == "all" ]]; then
    ...
    curl -sL "https://spreadsheets.google.com/feeds/download/spreadsheets/Export?key="$genomesId"&hl=en&exportFormat=tsv" | tr -d "\r" > $DIR/$name/genomes.tsv && echo "" >> $DIR/$name/genomes.tsv

    rm -f $DIR/$name/genomes.txt
    
    awk -F "\t" '{if(NR==1){split($0,xx,"\t"); nf=NF} ; if(NR>1){for (i = 1; i <= nf; i++) print xx[i]" "$i; print ""  }}' $DIR/$name/genomes.tsv | grep -v " NA$" | grep -v "^_" > $DIR/$name/genomes.txt
